...
def limpiar_pantalla():
    ...
    # El comando está harcodeado y no hay riesgo de inyección
...
def banner():
    ...
    p4 = f"{BLUE} |   ||    |     /_____/ |    |   |  | \\// __ \\\\  "
    p4 += f"\\___|    <\\  ___/|  | \\/ {RESET}"
    p5 = f"{BLUE} |___||____|             |____|   |__|  (____  /\\"
    p5 += f"___  >__|_ \\\\___  >__|    {RESET}"
    p6 = f"{BLUE}                                             \\/     "
    p6 += f"\\/     \\/    \\/        {RESET}"
    print(p1)
    print(p2)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'fail':
                msg = data.get('message', 'IP inválida o privada')
                print(f"{RED}[!]{RESET} Error: {msg}")
                return None
            ...
            nombre_archivo = os.path.join(
                carpeta, f"IP_{ip_address}_{timestamp}.txt"
            )
            ...
            with open(nombre_archivo, "w", encoding="utf-8") as file:
                ...
                maps_url += f"{data.get('lat')},{data.get('lon')}"
            ...
            country_line = f"{data.get('country', 'N/A')} "
            country_line += f"({data.get('countryCode', 'N/A')})"
            print(f"{BOLD}País:{RESET} {country_line}")
            ...
            print(f"{BOLD}Coordenadas:{RESET} {coords}")
            ...
            print(f"\n{GREEN}[✓]{RESET} Resultados guardados en: "
                  f"{nombre_archivo}")
            print(f"{GREEN}[✓]{RESET} Ver en Google Maps: {maps_url}")
...
def geolocalizar_ip_metodo2(ip_address):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            carpeta = crear_carpeta_resultados()
            nombre_archivo = os.path.join(
                carpeta, f"IP_{ip_address}_metodo2_{timestamp}.txt"
            )
...
def analizar_telefono():
    ...
    promp_str += f"[2] Ingresar por partes\nOpción: {RESET}"
    ...
    try:
        ...
        fmt_int = phonenumbers.format_number(
        ...
        fmt_nac = phonenumbers.format_number(
        ...
        tipos = {
            0: "Fijo", 1: "Móvil", 2: "Fijo o Móvil", 3: "Gratuito",
            4: "Tarifa Premium", 5: "Costo Compartido", 6: "VoIP",
            7: "Número Personal", 8: "Pager", 9: "UAN", 10: "Desconocido"
        }
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        carpeta = crear_carpeta_resultados()
        tel_id = f"{phone_obj.country_code}{phone_obj.national_number}"
        nombre_archivo = os.path.join(
            carpeta, f"Telefono_{tel_id}_{timestamp}.txt"
        )
        ...
        with open(nombre_archivo, "w", encoding="utf-8") as f:
            ...
            f.write("="*60 + "\n\n")
            now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            f.write(f"Fecha de consulta: {now_str}\n\n")
            f.write(f"Número válido: {'Sí' if es_valido else 'No'}\n")
            f.write(f"Número posible: {'Sí' if es_posible else 'No'}\n\n")
            f.write(f"Formato internacional: {fmt_int}\n")
            f.write(f"Formato E.164: {fmt_e164}\n")
            f.write(f"Formato nacional: {fmt_nac}\n\n")
            ...
            tz_str = ', '.join(zonas_horarias) if zonas_horarias else 'N/A'
            f.write(f"Zonas horarias: {tz_str}\n")
        ...
        # Formatos
        print(f"\n{BOLD}Formatos:{RESET}")
        print(f"  Internacional: {fmt_int}")
        print(f"  E.164: {fmt_e164}")
        print(f"  Nacional: {fmt_nac}")
